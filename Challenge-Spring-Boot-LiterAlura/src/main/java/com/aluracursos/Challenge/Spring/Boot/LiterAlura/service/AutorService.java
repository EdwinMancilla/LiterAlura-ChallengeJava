...
public class AutorService {
    ...
    public List<Autor> listaAutoresVivosEnAno(int ano){
    ...
    public Autor actualizarAutor(Long id, Autor autorDetalles) {
        Autor autor = autorRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("Autor no Encpontrado"));
        autor.setNombre(autorDetalles.getNombre());
        autor.setAnoNacimiento(autorDetalles.getAnoNacimiento());
        autor.setAnoNacimiento(autorDetalles.getAnoFallecimiento());
        return autorRepository.save(autor);
    }
